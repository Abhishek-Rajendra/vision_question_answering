...
def showExample(train=True, id=None, dataset=None):
    if train:
        data = dataset["train"]
    else:
        data = dataset["test"]
...
def main(config_path: Text, img_path: Text, question: Text, show_img: bool) -> None:
    ...
    checkpoint = os.path.join(config["training"]["checkpoint"], config["model"]["name"], config["inference"]["checkpoint"]+".ckpt")
...
if __name__ == "__main__":
    ...
    args_parser.add_argument('--show_img', action="store_true", help="show an example image & question")
    ...
    main(args.config, args.img_path, args.question, args.show_img)